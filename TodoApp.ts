class TodoApp {
    ...
    initialize(el: { getElementsByClassName: (arg0: string) => any[]; }) {
        ...
        addTodoFormEl.addEventListener('submit', function(evnt: { preventDefault: () => void; }) {
            _this.addTodo(addTodoNameEl.value)
            addTodoNameEl.value = '';
            evnt.preventDefault();
        });

        todoListEl.addEventListener('todo-toggle', function(evnt: { detail: { todoId: any; }; }) {
